///////////////////////////////////////////////////////////////////////////////
//
//       MODULE: axil_m_scoreboard
//       AUTHOR: zack
// ORGANIZATION: fsic
//      CREATED: 2023/05/17
///////////////////////////////////////////////////////////////////////////////

class axil_m_scoreboard;
    virtual axil_m_interface.slave intf;
    axil_m_scenario scnr_scrbd[2];
    mb_axi mb_scrbd[2];
    static int succ_cnt, fail_cnt;
    axil_m_scenario scn_wr_tr_q[$], scn_rd_tr_q[$], mon_wr_tr_q[$], mon_rd_tr_q[$], scn_wr_tr, scn_rd_tr, mon_wr_tr, mon_rd_tr;

    function new(virtual axil_m_interface.slave intf, mb_axi mb_gen, mb_mon);
        this.intf = intf;                         
    ...
    virtual task compare_trans();
        fork
            ...
            while(1)begin
                if((scn_wr_tr_q.size() != 0) && (mon_wr_tr_q.size() != 0))begin
                    ...
                    if(scn_wr_tr.compare(mon_wr_tr))begin
                        ...
                        $display($sformatf("[ERROR] trans %6d compare fail", scn_wr_tr.trans_id));
                        scn_wr_tr.display();
                        mon_wr_tr.display();
                        fail_cnt +=1;
                    end
            ...
            while(1)begin
                if((scn_rd_tr_q.size() != 0) && (mon_rd_tr_q.size() != 0))begin
                    ...
                    if(scn_rd_tr.compare(mon_rd_tr))begin
                        ...
                        $display($sformatf("[ERROR] trans %6d compare fail", scn_rd_tr.trans_id));
                        scn_rd_tr.display();
                        mon_rd_tr.display();
                        fail_cnt +=1;
                    end
            ...
            while(1)begin
                if((succ_cnt + fail_cnt) >= axil_m_scenario_gen::PKT_NUM) break;
                else begin
                    @(posedge intf.axi_aclk);
                end
            end
