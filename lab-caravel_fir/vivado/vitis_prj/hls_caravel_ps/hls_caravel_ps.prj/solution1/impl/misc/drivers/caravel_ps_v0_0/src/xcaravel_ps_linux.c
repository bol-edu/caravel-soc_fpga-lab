...
typedef struct {
    ...
    XCaravel_ps_uio_map maps[ MAX_UIO_MAPS ];
} XCaravel_ps_uio_info;

/***************** Variable Definitions **************************************/
static XCaravel_ps_uio_info uio_info;
...
static int uio_info_read_name(XCaravel_ps_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCaravel_ps_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCaravel_ps_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCaravel_ps_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCaravel_ps_Initialize(XCaravel_ps *InstancePtr, const char* InstanceName) {
	XCaravel_ps_uio_info *InfoPtr = &uio_info;
