...
*/
    ...
    */
    ...
    blkRamb#(.SIZE(mem_sizes), .BIT_WIDTH(DQ_BITS))
    Bank0(
        .clk(Sys_clk), 
        .we(bwen[0]), 
        .re(bren[0]), 
        ...
        */
        if(!Rst_n)begin
            ...
            if (Prech_enable == 1'b1) begin
                ...
                Col_addr[Cas_latency - 1] <= {Addr[12:11], Addr[9:0]};//Cas_latency - 1
                Bank_addr[Cas_latency - 1] <= Ba;//Cas_latency - 1
            end    
            else if (Write_enable == 1'b1) begin
                Command[Cas_latency - 1] <= `WRITE;//0
                Col_addr[Cas_latency - 1] <= {Addr[12:11], Addr[9:0]};//0
    ...
    // Detect Read or Write command
            if (Read_enable || Command[0] == `WRITE) begin//Command[2] == `READ
                if(Read_enable)begin//Command[2] == `READ
                    Bank <= Ba;//Bank_addr[2]
                    //Col <= Addr;//Col_addr[2]
                    Col_brst <= {Addr[12:11], Addr[9:0]};//Col_addr[2]
                    case (Ba)//Bank_addr[2]
...
module blkRamb #(parameter SIZE = 8192, parameter BIT_WIDTH = 8)(clk, we, re, waddr, raddr, d, q);
