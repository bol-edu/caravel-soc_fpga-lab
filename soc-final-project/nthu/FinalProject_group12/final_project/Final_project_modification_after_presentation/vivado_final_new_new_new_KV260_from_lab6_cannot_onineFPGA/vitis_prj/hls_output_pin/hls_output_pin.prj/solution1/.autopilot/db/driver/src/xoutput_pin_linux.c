...
typedef struct {
    ...
    XOutput_pin_uio_map maps[ MAX_UIO_MAPS ];
} XOutput_pin_uio_info;

/***************** Variable Definitions **************************************/
static XOutput_pin_uio_info uio_info;
...
static int uio_info_read_name(XOutput_pin_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XOutput_pin_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XOutput_pin_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XOutput_pin_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XOutput_pin_Initialize(XOutput_pin *InstancePtr, const char* InstanceName) {
	XOutput_pin_uio_info *InfoPtr = &uio_info;
