...
#(parameter
    ...
    output wire [63:0]                   b,
    output wire [63:0]                   c,
    output wire [31:0]                   a_row,
    output wire [31:0]                   a_col,
    output wire [31:0]                   b_col,
...
localparam
    ...
    ADDR_C_DATA_1     = 7'h2c,
    ADDR_C_CTRL       = 7'h30,
    ADDR_A_ROW_DATA_0 = 7'h34,
    ADDR_A_ROW_CTRL   = 7'h38,
    ADDR_A_COL_DATA_0 = 7'h3c,
    ADDR_A_COL_CTRL   = 7'h40,
    ADDR_B_COL_DATA_0 = 7'h44,
    ADDR_B_COL_CTRL   = 7'h48,
    WRIDLE            = 2'd0,
    WRDATA            = 2'd1,
    WRRESP            = 2'd2,
    WRRESET           = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_b = 'b0;
    reg  [63:0]                   int_c = 'b0;
    reg  [31:0]                   int_a_row = 'b0;
    reg  [31:0]                   int_a_col = 'b0;
    reg  [31:0]                   int_b_col = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_A_ROW_DATA_0: begin
                    rdata <= int_a_row[31:0];
                end
                ADDR_A_COL_DATA_0: begin
                    rdata <= int_a_col[31:0];
                end
                ADDR_B_COL_DATA_0: begin
                    rdata <= int_b_col[31:0];
                end
            endcase
        end
    end
...
assign b                 = int_b;
assign c                 = int_c;
assign a_row             = int_a_row;
assign a_col             = int_a_col;
assign b_col             = int_b_col;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_A_ROW_DATA_0)
            int_a_row[31:0] <= (WDATA[31:0] & wmask) | (int_a_row[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_A_COL_DATA_0)
            int_a_col[31:0] <= (WDATA[31:0] & wmask) | (int_a_col[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_B_COL_DATA_0)
            int_b_col[31:0] <= (WDATA[31:0] & wmask) | (int_b_col[31:0] & ~wmask);
    end
end
