...
//------------------------Parameter----------------------
`define TV_IN_gmem2 "../tv/cdatafile/c.mmult.autotvin_gmem2.dat"
`define TV_OUT_gmem2 "../tv/rtldatafile/rtl.mmult.autotvout_gmem2.dat"
...
parameter gmem2_C_DATA_BITWIDTH = 32'd 32;
parameter gmem2_mem_depth = 32'd 256;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem2_mem_depth) begin
                $display ("/home/ubuntu/Desktop/2024_Fall_NTU_AAHLS_SP/LabB_Systolic_Array/Vitis_HLS/SA_broadcasting/solution1/sim/verilog/AESL_axi_master_gmem2.v: Write request address %d exceed AXI master gmem2 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem2_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem2_mem_depth) begin
                $display ("/home/ubuntu/Desktop/2024_Fall_NTU_AAHLS_SP/LabB_Systolic_Array/Vitis_HLS/SA_broadcasting/solution1/sim/verilog/AESL_axi_master_gmem2.v: Read request address %d exceed AXI master gmem2 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem2_mem_depth); 
                $finish;
            end
...
initial begin : write_file_proc 
  ...
  integer mem_page; 
  integer gmem2_bitwidth; 
  reg [gmem2_C_DATA_BITWIDTH - 1 : 0] mem_tmp; 
  reg [gmem2_DATA_BITWIDTH - 1 : 0] tmp_data;
  reg [63:0] bin_data;
  transaction_num = 0; 
  gmem2_bitwidth = gmem2_C_DATA_BITWIDTH; 
  count_seperate_factor_by_bitwidth (gmem2_bitwidth , factor);
  while(1) begin 
      ...
      mem_page = transaction_num % mem_page_num ;
      # 0.1;
      fp = $fopen(`TV_OUT_gmem2, "ab"); 
      if(fp == 0) begin       // Failed to open file 
          $display("Failed to open file \"%s\"!", `TV_OUT_gmem2); 
          $finish; 
      end 
      bin_data = gmem2_mem_depth;
      write_binary(fp,bin_data,64);
      for(i = 0; i < (gmem2_mem_depth - gmem2_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              case(mem_page)
                  0 : tmp_data = gmem2_mem_0[i/factor];
                  1 : tmp_data = gmem2_mem_1[i/factor];
                  2 : tmp_data = gmem2_mem_2[i/factor];
                  3 : tmp_data = gmem2_mem_3[i/factor];
          ...
          if (factor == 1) begin
              case(mem_page)
                  0 : mem_tmp = gmem2_mem_0[i];
                  1 : mem_tmp = gmem2_mem_1[i];
                  2 : mem_tmp = gmem2_mem_2[i];
                  3 : mem_tmp = gmem2_mem_3[i];
                  default: $display("The page_num of write file is not valid!");
              endcase
          bin_data = mem_tmp;
      ...
      if (factor == 4) begin
          case(mem_page)
              0 : tmp_data = gmem2_mem_0[gmem2_mem_depth/factor];
              1 : tmp_data = gmem2_mem_1[gmem2_mem_depth/factor];
              2 : tmp_data = gmem2_mem_2[gmem2_mem_depth/factor];
              3 : tmp_data = gmem2_mem_3[gmem2_mem_depth/factor];
              default: $display("The page_num of write file is not valid!");
          endcase
          if ((gmem2_mem_depth - 1) % factor == 2) begin
          ...
          if ((gmem2_mem_depth - 1) % factor == 1) begin
              bin_data = tmp_data[7:0];
              write_binary(fp,bin_data,8);
              bin_data = tmp_data[15:8];
              write_binary(fp,bin_data,8);
          end
          if ((gmem2_mem_depth - 1) % factor == 0) begin
              bin_data = tmp_data;
              write_binary(fp,bin_data,8);
          end
      end
      if (factor == 2) begin
          if ((gmem2_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  0 : tmp_data = gmem2_mem_0[gmem2_mem_depth/factor][15:0];
                  1 : tmp_data = gmem2_mem_1[gmem2_mem_depth/factor][15:0];
                  2 : tmp_data = gmem2_mem_2[gmem2_mem_depth/factor][15:0];
                  3 : tmp_data = gmem2_mem_3[gmem2_mem_depth/factor][15:0];
                  default: $display("The page_num of write file is not valid!");
              endcase
              bin_data = tmp_data;
              write_binary(fp,bin_data,16);
