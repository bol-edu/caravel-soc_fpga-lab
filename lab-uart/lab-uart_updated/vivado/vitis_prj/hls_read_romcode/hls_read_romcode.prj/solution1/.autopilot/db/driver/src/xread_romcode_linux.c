...
typedef struct {
    ...
    XRead_romcode_uio_map maps[ MAX_UIO_MAPS ];
} XRead_romcode_uio_info;

/***************** Variable Definitions **************************************/
static XRead_romcode_uio_info uio_info;
...
static int uio_info_read_name(XRead_romcode_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRead_romcode_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRead_romcode_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRead_romcode_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRead_romcode_Initialize(XRead_romcode *InstancePtr, const char* InstanceName) {
	XRead_romcode_uio_info *InfoPtr = &uio_info;
